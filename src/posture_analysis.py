...
class PostureAnalyzer:
    ...
    CORREÇÃO: Separação estrita entre lógica de contagem e feedback de qualidade.
    """
    def __init__(self, exercise_config_path, pose_detector):
        ...
        # --- LÓGICA DE CONTAGEM (QUANTITATIVA) ---
        self.rep_state = "up"
        self.counter = 0
        self.rep_quality = True
        self.actual_rep_errors = set()

        # --- LÓGICA DE FEEDBACK (QUALITATIVA) ---
        self.feedback = "Inicie o exercicio."
        self.feedback_type = "INFO"
        self.movement_phase = "INICIANDO"
        # O timer de repetição foi removido daqui para não poluir o feedback de postura
    ...
    def _analyze_squat_phase(self, angles, visibilities):
        ...
        if knee_angle is None or hip_angle is None: return "INDETERMINADO"
        if knee_angle > STAND_THRESHOLD: return "EM PE"
        ...
        if is_knee_in_range and is_hip_in_range and is_hip_deeper_than_knee: return "AGACHADO"
        return "TRANSICAO"

    def analyze(self, keypoints, image_shape, reporter):
        """
        Executa a análise frame a frame.
        Retorna: Dicionário de ângulos calculados.
        """
        if not keypoints:
        ...
        # Validação da qualidade da repetição
    ...
    def detect_body_orientation(self, keypoints, image_shape, vert_threshold=0.6):
        if not keypoints or image_shape is None: return "INDETERMINADO"
        h, w = image_shape[:2]
        ...
        if max_idx >= len(keypoints): return "INDETERMINADO"
        
        joint_points = {
            'left_shoulder': keypoints[left_shoulder_idx] if keypoints[left_shoulder_idx][3] > 0.5 else None,
        ...
        elif joint_points['left_shoulder']: superior_point = np.array(joint_points['left_shoulder'][:2])
        elif joint_points['right_shoulder']: superior_point = np.array(joint_points['right_shoulder'][:2])
        ...
        elif joint_points['left_hip']: inferior_point = np.array(joint_points['left_hip'][:2])
        elif joint_points['right_hip']: inferior_point = np.array(joint_points['right_hip'][:2])
        else: return "INDETERMINADO"
        ...
        return "EM PE (Orientacao)" if verticality_ratio > vert_threshold else "HORIZONTAL (FLEXAO)"
    ...
    def _update_rep_counter(self, main_angle_value, reporter):
        ...
        if self.rep_state == 'up' and main_angle_value < down_threshold:
            ...
            self.rep_state = 'up'
            # REMOVIDO: self.rep_complete_feedback_end_time = time.time() + 2
    
    def _get_posture_feedback(self, keypoints, angles, visibilities, active_angle_name):
        # ... (Mantém o código original de verificação de regras) ...
        # (Omitido para brevidade, a lógica de regras permanece idêntica ao original)
        ...
        for rule in self.rules['feedback']:
            ...
            if apply_when and current_phase != apply_when and apply_when != "both": continue
            ...
            # Lógica Segment Parallelism (Adicionada conforme código original)
            ...
            # Lógica Vertical Comparison
                ...
                if vis1 > 0.65 and vis2 > 0.65:
                    ...
                    if rule['condition'] == 'is_below_or_level' and y1 < y2: return rule['message'], "ATENCAO"
            
            # Lógica Angle Offset
            elif rule_type == 'angle_offset':
                base_angle_name, offset_angle_name = rule['base_angle'], rule['offset_angle']
                if active_side_prefix:
                ...
                base_val, offset_val = angles.get(base_angle_name), angles.get(offset_angle_name)
                vis1, vis2 = visibilities.get(base_angle_name, 0), visibilities.get(offset_angle_name, 0)
                if base_val is not None and offset_val is not None and vis1 > 0.65 and vis2 > 0.65:
                    offset = base_val - offset_val
                    expected = rule['expected_offset_range']
                    if not (expected['min'] <= offset <= expected['max']): return rule['message'], "ATENCAO"
